...
#define dodbs_t_default(l)                                                     \
  { .array = {0}, .len = l }
#define dodbs_t_decl(bs, l) dodbs_t(l) bs = dodbs_t_default(l)
#define dodbs_t_decl_ptr(bs, l)                                                \
  dodbs_t(l) *bs = memcpy(malloc(sizeof(*bs)),                                 \
                          &(dodbs_t(l))dodbs_t_default(l), sizeof(*bs))

// Constructors and destructors
#define dodbs_init(bs, u)                                                      \
  do {                                                                         \
...
// Helpers
#define dodbs_print(bs)                                                        \
  do {                                                                         \
    size_t _sz_ = DODBS_ARR_SZ(bs);                                            \
    for (size_t _i_ = 0; _i_ < _sz_; ++_i_) {                                  \
      for (size_t _j_ = 0; _j_ < DODBS_T_BIT; ++_j_) {                         \
        if (_i_ >= (_sz_ - 1) && _j_ >= (bs).len)                              \
          break;                                                               \
        printf("%u",                                                           \
               BIT_CHECK((bs).array[_sz_ - _i_ - 1], DODBS_T_BIT - _j_ - 1)    \
                   ? 1                                                         \
                   : 0);                                                       \
      }                                                                        \
    }                                                                          \
  } while (0)
#define dodbs_equals(bs1, bs2)                                                 \
  dodbs_equals_((bs1).array, (bs1).len, (bs2).array, (bs2).len,                \
                DODBS_ARR_SZ(bs1))

// Bit access
#define dodbs_count(bs) dodbs_count_((bs).array, (bs).len, DODBS_ARR_SZ(bs))
#define dodbs_size(bs) (DODBS_ARR_SZ(bs) * DODBS_T_BIT)
#define dodbs_test(bs, pos)                                                    \
  BIT_CHECK((bs).array[pos / DODBS_T_BIT], pos % DODBS_T_BIT)
...
// Bit operations
#define dodbs_set(bs, pos)                                                     \
  BIT_SET((bs).array[pos / DODBS_T_BIT], pos % DODBS_T_BIT)
#define dodbs_setall(bs) ((void)memset((bs).array, 0xff, sizeof((bs).array)))
#define dodbs_reset(bs, pos)                                                   \
  BIT_CLEAR((bs).array[pos / DODBS_T_BIT], pos % DODBS_T_BIT)
#define dodbs_resetall(bs) ((void)memset((bs).array, 0, sizeof((bs).array)))
#define dodbs_flip(bs, pos)                                                    \
  BIT_FLIP((bs).array[pos / DODBS_T_BIT], pos % DODBS_T_BIT)
#define dodbs_flipall(bs)                                                      \
  do {                                                                         \
    size_t _sz_ = DODBS_ARR_SZ(bs);                                            \
    for (size_t _i_ = 0; _i_ < _sz_; ++_i_) {                                  \
      for (size_t _j_ = 0; _j_ < DODBS_T_BIT; ++_j_) {                         \
...
// Bitset operations
#define dodbs_to_string(bs)                                                    \
  dodbs_to_string_(bs.array, (bs).len, DODBS_ARR_SZ(bs))
#define dodbs_to_ulong(bs) dodbs_to_ulong_(bs.array)
#define dodbs_to_ullong(bs) dodbs_to_ullong_(bs.array)

// Internal functions
static inline bool dodbs_equals_(const dodbs_storage_t *bytes1, size_t l1,
                                 const dodbs_storage_t *bytes2, size_t l2,
                                 size_t sz) {
  if (l1 != l2)
    return false;
  size_t i = 0;
...
static inline size_t dodbs_count_(const dodbs_storage_t *array, size_t l,
                                  size_t sz) {
  size_t count = 0;
...
static inline bool dodbs_any_(const dodbs_storage_t *array, size_t l,
                              size_t sz) {
  for (size_t i = 0; i < sz; ++i) {
    for (size_t j = 0; j < DODBS_T_BIT; ++j) {
      if (i >= (sz - 1) && j >= l)
...
static inline bool dodbs_all_(const dodbs_storage_t *array, size_t l,
                              size_t sz) {
  for (size_t i = 0; i < sz; ++i) {
    for (size_t j = 0; j < DODBS_T_BIT; ++j) {
      if (i >= (sz - 1) && j >= l)
...
static inline char *dodbs_to_string_(const dodbs_storage_t *array, size_t l,
                                     size_t sz) {
  char *str = malloc(l + 1);
  str[l] = '\0';
  for (size_t i = 0; i < sz; ++i) {
    for (size_t j = 0; j < DODBS_T_BIT; ++j) {
      if (i >= (sz - 1) && j >= l)
        break;
      str[(i * DODBS_T_BIT) + j] =
          (BIT_CHECK(array[sz - i - 1], DODBS_T_BIT - j - 1) == 1) ? '1' : '0';
    }
  }
  return str;
}
...
dodbs_to_ullong_(const dodbs_storage_t *array) {
